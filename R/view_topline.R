#' Calculate topline results and generate report
#'
...
#' then outputs a report of data.
#'
#' Two output types are available: pdf and csv.
#'
#' Selecting PDF generates a clean report with one table per question and one
#' row per answer choice. If a .Rmd file is specified as `template`, a custom
#' template is used to render the report. The custom template can retrieve data
#' from `tables`, which is a list of tibbles available in the rendering
#' environment. If found in `codebook`, titles and captions are added to the
#' default report and can be accessed by a custom template as the tibble
#' `codebook`; see below for details.
#'
#' Selecting CSV generates a single, clean comma-delimited file with one row
#' per answer choice. Specifying `template` has no effect other than to produce
#' a warning. Columns found in `codebook` will be joined to the output by
#' question number.
#'
#' The file specified as `codebook` must be a **tab-delimited** file and
#' include a `QN` column containing the question numbers corresponding to
#' columns in the cleaned data and a `type` column specifying the question type
#' (`mc` for multiple-choice, `ms` for multiple-select, `no` for numeric, or
#' `ft` for free-text). If you have multiple-select questions on your survey
#' (select all that apply), your data likely has multiple columns for these
#' questions; `codebook` should only have one entry for the full question. For
#' example, while your data may have columns `Q01_1`, `Q01_2`, and `Q01_3` for
#' three answer choices, your `codebook` file should only have a `Q01` row. For
#' this reason, `codebook` here is different from in `setup` and probably can't
#' be the same file. For PDF output using the default template, you may also
...
view_topline <- function(codebook,
  ...
  for(i in 1:nrow(cb)){
      ...
      if("wt" %in% names(tosum) && !(q %in% exclude)){
        ...
        tables[[i]] <- tosum %>%
          ...
          dplyr::summarize(
            count = dplyr::n(),
            pct = sum(wt) / wt_tot,
            lci = pct - 1.96 * sqrt(pct * (1 - pct) / n_tot),
            uci = pct + 1.96 * sqrt(pct * (1 - pct) / n_tot)
          ) %>%
          dplyr::mutate(
    ...
    if(type == "ms"){
      ...
      for(h in 1:ncol(dplyr::select(res, tidyselect::starts_with(q)))){
        tosum <- dplyr::select(
          res, Q = tidyselect::starts_with(q)[h],
          wt = tidyselect::any_of(wt)
        )
        if("wt" %in% names(tosum) && !(q %in% exclude)){
    ...
    if(type == "no"){
      ...
      ### Pull variable
      var <- dplyr::pull(res, q) %>% as.numeric()
  ...
  if(make == "pdf"){
    ...
    if(is.na(template)){
      rmarkdown::render(
        input = system.file("auto", "view_topline.Rmd", package = "clusteR"),
        output_dir = paste0(getwd(), "/Survey Data"),
        output_file = "Topline Report.pdf",
        envir = knit_env,
        quiet = T
      )
    } else {
    ...
    if(Sys.getenv("RSTUDIO") == "1"){
      rstudioapi::viewer("Survey Data/Topline Report.pdf")
    } else {
      rlang::inform(
        message = c(
          cli::style_bold("Report generated."),
          "i" = paste0("See ", cli::style_underline("Survey Data/Topline Report.pdf"), ".")
        )
      )
    }
    return()
